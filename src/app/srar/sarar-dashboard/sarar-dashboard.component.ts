...
export class SararDashboardComponent implements OnInit {
  ...
  @ViewChild('enhancedChartCanvas') enhancedChartCanvas!: ElementRef;
  ...
  private enhancedChart: Chart | null = null;
          ...
          tooltip: {
            ...
            callbacks: {
              label: function(context) {
                ...
                } else if (label && label.includes('Distance')) {
                  return label + ': ' + value + ' km';
                } else if (label && label.includes('Speed')) {
                  return label + ': ' + value + ' knots';
                }
                return label + ': ' + value;
              }
            }
  ...
  createOperationalReadinessChart() {
    ...
    this.charts['operationalReadiness'] = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: this.operationalReadinessData.datasets.map((dataset: any, index: number) => ({
          ...dataset,
          borderWidth: 3,
          borderRadius: 12,
          hoverBackgroundColor: dataset.backgroundColor.replace('0.8', '1'),
          hoverBorderColor: '#ffffff',
          hoverBorderWidth: 2
        }))
      },
    ...
    this.charts['operationalReady'] = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          borderWidth: 3,
          borderRadius: 12,
          hoverBackgroundColor: shipData.operationalReady.backgroundColor.replace('0.8', '1'),
          hoverBorderColor: '#ffffff',
          hoverBorderWidth: 2
        }]
      },
  ...
  // Enhanced Chart.js Features - Similar to Maintop Dashboard
  createEnhancedChart() {
    const ctx = this.enhancedChartCanvas.nativeElement.getContext('2d');
    
    if (this.enhancedChart) {
      this.enhancedChart.destroy();
    }
    
    // Sample data for enhanced chart
    const labels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
    const operationalData = [85, 78, 92, 88, 95, 89];
    const maintenanceData = [12, 18, 8, 15, 5, 11];
    const readinessData = [95, 88, 96, 92, 98, 94];
    
    this.enhancedChart = new Chart(ctx, {
      type: 'line' as ChartType,
      data: {
        labels: labels,
        datasets: [
          {
            ...
            pointBorderWidth: 3 as const,
            pointRadius: 8 as const,
            pointHoverRadius: 12 as const,
          ...
          },
          {
            ...
            borderDash: [8, 4] as const,
            fill: false as const,
            tension: 0.4 as const,
            pointBackgroundColor: '#3B82F6',
            pointBorderColor: '#ffffff',
            pointBorderWidth: 2 as const,
            pointRadius: 6 as const,
            pointHoverRadius: 10 as const,
            pointHoverBackgroundColor: '#2563EB',
            pointHoverBorderColor: '#ffffff'
          },
          {
            label: 'Overall Fleet Readiness (%)',
            data: readinessData,
            borderColor: '#8B5CF6',
      ...
      options: this.createEnhancedTrendChart()
    });
  }

  createEnhancedTrendChart() {
    // This method demonstrates advanced Chart.js features
    // that can be used to create more sophisticated visualizations
  ...
  // Enhanced Chart Border Colors
  getEnhancedChartBorderColors() {
    return {

// src/app/sfd/sfd-dashboard/sfd-dashboard.component.ts
  ...
  loadDashboardData(filters: DashboardFilters = {}): void {
    ...
    combineLatest([
      ...
      next: ([equipment, ships, commands, alerts]) => {
        ...
        // Update KPI metrics with actual values
        this.kpiMetrics[0].value = this.totalActiveShips;
        this.kpiMetrics[1].value = this.operationalReadinessIndex + '%';
        this.kpiMetrics[2].value = this.criticalEquipmentHealthScore;
        this.kpiMetrics[3].value = this.equipmentAvailabilityIndex + '%';
